...
def train():
    ...
    if model_args.cache_dir:
        ...
        dataset_name = (
            model_args.cache_dir.split("/")[1]
            if "/" in model_args.cache_dir
            else "mimic3"
        )
    ...
    if model_args.peft_path is not None:  # for test model
        ...
        if training_args.resume_from_checkpoint is not None:
            model = PeftModelForCLS.from_pretrained(
                model, model_args.peft_path, is_trainable=True
            )
        else:
            model = PeftModelForCLS.from_pretrained(
                model, model_args.peft_path, is_trainable=False
            )
    ...
    # Fix: Ensure pad_token_id is set for both PEFT model and its base model
    pad_token_id = tokenizer.pad_token_id
    model.config.pad_token_id = pad_token_id
    # Also set on base model for PEFT wrapper
    ...
    if training_args.do_train:
        preprocess_func = llama_train_cls(
            data_args, model_args, tokenizer, ehr_tokenizer
        )
    else:
        preprocess_func = llama_eval_cls(
            data_args, model_args, tokenizer, ehr_tokenizer
        )
    ...
    trainer = MedRecTrainer(
        ...
        callbacks=(
            [SavePeftModelCallback] if isinstance(model, PeftModel) else None
        ),  # substitute the original model saver
    )
    ...
    if training_args.do_predict:
        ...
        if trainer.is_world_process_zero():
            ...
            drug_codes_json_file = os.path.join(
                training_args.output_dir, "drug_codes_comparison.json"
            )
            drug_codes_csv_file = os.path.join(
                training_args.output_dir, "drug_codes_comparison.csv"
            )
            ...
            with open(drug_codes_csv_file, "w", encoding="utf-8", newline="") as f:
                ...
                for i in range(len(drug_code_results["subject_ids"])):
                    ...
                    true_codes = "; ".join(
                        sorted(drug_code_results["true_drug_codes"][i])
                    )  # Join with semicolon + space
                    pred_codes = "; ".join(
                        sorted(drug_code_results["pred_drug_codes"][i])
                    )  # Join with semicolon + space
            ...
            print(
                f"   - Average true drugs per patient: {sum(len(codes) for codes in drug_code_results['true_drug_codes']) / len(drug_code_results['true_drug_codes']):.2f}"
            )
            print(
                f"   - Average predicted drugs per patient: {sum(len(codes) for codes in drug_code_results['pred_drug_codes']) / len(drug_code_results['pred_drug_codes']):.2f}"
            )
