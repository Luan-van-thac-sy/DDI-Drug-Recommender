...
from utils.utils import t2n, metric_report, metric_report_group, apply_ddi_constraints
import time
...
class MedRecTrainer(Trainer):
    def __init__(self, args, logger, device, generator):
        ...
        self.ehr_adj = pickle.load(
            open(os.path.join(data_dir, "ehr_adj_final.pkl"), "rb")
        )
        self.ddi_adj = pickle.load(
            open(os.path.join(data_dir, "ddi_A_final.pkl"), "rb")
        )
        self.ddi_mask_H = pickle.load(
            open(os.path.join(data_dir, "ddi_mask_H.pkl"), "rb")
        )
        self.molecule = pickle.load(
            open(os.path.join(data_dir, "atc3toSMILES.pkl"), "rb")
        )
    ...
    def _create_model(self):
        ...
        if self.args.model_name == "leader":
            self.model = LEADER(
                config, self.args, self.tokenizer, self.device, self.profile_tokenizer
            )
        else:
            raise ValueError
    ...
    def eval(self, epoch=0, test=False):
        ...
        acc_container_group = metric_report_group(
            self.logger,
            np.concatenate(rx_y_preds, axis=0),
            np.concatenate(rx_y_trues, axis=0),
    ...
    def test(self):
        ...
        # Apply DDI constraints at inference if enabled
        ddi_constraint_enabled = getattr(self.args, "ddi_constraint", False)
        if ddi_constraint_enabled:
            self.logger.info("Applying DDI constraints at inference...")
            rx_y_preds_concat = np.concatenate(rx_y_preds, axis=0)
            rx_y_preds_constrained = apply_ddi_constraints(
                rx_y_preds_concat, self.ddi_adj, threshold=self.args.therhold
            )
            rx_y_preds_for_eval = rx_y_preds_constrained
            rx_y_preds_concat_raw = np.concatenate(rx_y_preds, axis=0)
        else:
            rx_y_preds_concat_raw = np.concatenate(rx_y_preds, axis=0)
            rx_y_preds_for_eval = rx_y_preds_concat_raw
        ...
        if ddi_constraint_enabled:
            self.logger.info("DDI constraint applied. Reporting constrained metrics.")
