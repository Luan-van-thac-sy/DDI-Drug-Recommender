"""
Phase 1: Data Preparation for DDI-Aware LEADER (MIMIC-IV)
Converts data4LLM.csv → LEADER JSONL format
Reads pre-built artifacts (voc, DDI, EHR) from Google Drive output folder
"""
...
# ============================================================
# Config — Google Drive / Colab paths
# ============================================================
...
def build_patient_records(rows, voc):
    ...
    for sid, visits in patients.items():
        ...
        # Append clinical NOTE if available (data4LLM.csv may not have NOTE column)
...
def main():
    print("=" * 60)
    print("Phase 1: Data Preparation for DDI-Aware LEADER (MIMIC-IV)")
    print("=" * 60)

    # Verify artifacts directory exists
    if not os.path.isdir(MIMIC4_ARTIFACTS):
        print(f"\nERROR: Artifacts directory not found: {MIMIC4_ARTIFACTS}")
        print("Make sure Google Drive is mounted at /content/drive/")
        sys.exit(1)

    # 1. Load vocabulary
    voc_path = os.path.join(MIMIC4_ARTIFACTS, "voc_final.pkl")
    ...
    # Load DDI adjacency for prompt enrichment
    ddi_path = os.path.join(MIMIC4_ARTIFACTS, "ddi_A_final.pkl")
    print(f"\n[1b] Loading DDI adjacency from {ddi_path}")
    ddi_adj = dill.load(open(ddi_path, "rb"))
    voc["ddi_adj"] = ddi_adj
    print(f"    ddi_adj shape: {ddi_adj.shape}")

    # Load MDC matrix for prompt enrichment
    ...
    # 8. Copy pre-built artifacts from Google Drive to project data dir
    ...
    for fname, dest in artifacts:
        src = os.path.join(MIMIC4_ARTIFACTS, fname)
    ...
    print("\n" + "=" * 60)
    print("Phase 1 COMPLETE! (MIMIC-IV)")
